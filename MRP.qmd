...
compare<-result %>% 
            mutate(Date = as.Date(Date),
                   Fulfill = ifelse(Inv_start/Demand < 1,round(Inv_start/Demand,3)*100,100),
                   Fulfill_safety = ifelse(Inv_start_safety/Demand < 1, round(Inv_start_safety/Demand,3)*100,100)) %>% 
  select (c(Date,
            Demand,
            Fulfill,
            Fulfill_safety,
...
reactable(
  ...
  columns = list(
    Fulfill = colDef(
      ...
      align = "center",
      cell = function(value) {
        # Format the value and append the '%' sign
        formatted_value <- paste0(format(value, nsmall = 1), "%")
        color <- ifelse(value <= 0, "#e00000", "#008000")
        htmltools::tags$span(formatted_value, style = paste("color:", color, "; font-weight: bold;"))
      }
    ),
